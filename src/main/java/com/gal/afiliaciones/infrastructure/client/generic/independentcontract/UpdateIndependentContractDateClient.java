...
/**
 * Update Independent Contract Date Client with automatic telemetry.
...
@Slf4j
public class UpdateIndependentContractDateClient {
    private final BusTokenService busTokenService;
    private final AffiliationProperties properties;

    public Object update(UpdateIndependentContractDateRequest request) {
        String url = properties.getIndependentContractDateUpdateUrl();
        log.info("Calling update independent contract date endpoint: {}", url);
        log.debug("Sending independent contract date update request payload: {}", safe(request));
