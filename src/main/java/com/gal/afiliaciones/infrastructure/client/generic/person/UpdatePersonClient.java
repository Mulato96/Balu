...
/**
 * Update Person Client v2 with automatic telemetry.
...
@Slf4j
public class UpdatePersonClient {
    private final BusTokenService busTokenService;
    private final AffiliationProperties properties;

    public Object update(PersonRequest request) {
        String url = properties.getBusUrlPerson();
        log.info("Calling update person endpoint: {}", url);
        log.debug("Sending person update request payload: {}", safe(request));
