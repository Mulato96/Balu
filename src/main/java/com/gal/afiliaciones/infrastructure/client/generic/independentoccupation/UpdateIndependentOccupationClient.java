...
/**
 * Update Independent Occupation Client with automatic telemetry.
...
@Slf4j
public class UpdateIndependentOccupationClient {
    private final BusTokenService busTokenService;
    private final AffiliationProperties properties;

    public Object update(UpdateIndependentOccupationRequest request) {
        String url = properties.getIndependentOccupationUpdateUrl();
        log.info("Calling update independent occupation endpoint: {}", url);
        log.debug("Sending independent occupation update request payload: {}", safe(request));
