...
/**
 * Update Worker Position Client with automatic telemetry.
...
@Slf4j
public class UpdateWorkerPositionClient {
    private final BusTokenService busTokenService;
    private final AffiliationProperties properties;

    public Object update(UpdateWorkerPositionRequest request) {
        String url = properties.getWorkerPositionUpdateUrl();
        log.info("Calling update worker position endpoint: {}", url);
        log.debug("Sending worker position update request payload: {}", safe(request));
