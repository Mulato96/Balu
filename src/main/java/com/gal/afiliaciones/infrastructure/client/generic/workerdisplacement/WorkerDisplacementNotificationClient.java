...
/**
 * Worker Displacement Notification Client with automatic telemetry.
 * Part of integrations v2 architecture.
 * 
 * HTTP tracking happens automatically via WebClient interceptor.
 */
@Component("workerDisplacementNotificationClient")
...
public class WorkerDisplacementNotificationClient {
    ...
    public Object notifyDisplacement(WorkerDisplacementNotificationRequest request) {
        String url = properties.getWorkerDisplacementNotificationUrl();
        log.info("Calling worker displacement notification endpoint: {}", url);
        log.debug("Sending worker displacement notification request payload: {}", safe(request));
