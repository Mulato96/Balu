...
/**
 * Insert Headquarters (Sede) Client with automatic telemetry.
...
@Slf4j
public class InsertHeadquartersClient {
    private final BusTokenService busTokenService;
    private final AffiliationProperties properties;

    public Object insert(InsertHeadquartersRequest request) {
        String url = properties.getInsertHeadquartersUrl();
        log.info("Calling insert headquarters endpoint: {}", url);
        log.debug("Sending headquarters insert request payload: {}", safe(request));
        ...
        try {
            ...
            log.warn("Error calling insert headquarters {}: code={}, message={}", url, code, message);
            throw ex;
        }
    }
