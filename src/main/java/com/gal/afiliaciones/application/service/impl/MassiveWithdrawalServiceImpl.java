    ...
    private UploadResponseDTO processExcelFile(MultipartFile file, Long employerId) {
        try (Workbook workbook = new XSSFWorkbook(file.getInputStream())) {
            ...
            for (int i = 1; i < totalRows; i++) { // Start from 1 to skip header
                ...
                if (errors.isEmpty()) {
                    try {
                        String workerDocumentType = row.getCell(3).getStringCellValue();
                        String workerDocumentNumber = row.getCell(4).getStringCellValue();

                        Affiliate affiliate = affiliateRepository
                                .findByIdentificationDocumentTypeAndIdentificationDocumentNumber(workerDocumentType, workerDocumentNumber)
                        ...
                        retirement.setRetirementDate(LocalDate.parse(row.getCell(6).getStringCellValue()));
